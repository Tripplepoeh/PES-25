...
statuscontrole::statuscontrole(logToDatabase* log, getFromDatabase* get): logger(log), getDatabase(get) {
...
void statuscontrole::processI2CData(const uint8_t* d, size_t n) {
    ...
    while (i < n)
        processI2CCommands(d, i, n); //Verwerke alle get commandos
}
...
void statuscontrole::processI2CCommands(const uint8_t* d,size_t& i,size_t n) { //Verwerk commandos van de slave
    ...
    if (getDatabase) {
        ...
        static std::string lastMenu;  // onthoud het laatst verzonden menu

        if (menu != lastMenu) {  // alleen verzenden als het menu is veranderd
            lastMenu = menu;     // update het laatst bekende menu

            responseBuffer.push_back(aid);
            for (char c : menu) {
                responseBuffer.push_back(static_cast<uint8_t>(c));  // zet elke char als byte in de buffer
            }
            responseBuffer.push_back(0);  // nul-terminator
        }
        // anders: stuur niks als het menu hetzelfde is
    }
